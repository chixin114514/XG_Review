...
function renderQuestion() {{
  ...
  const correctText = Core.normalizeAnswer(question.correct_answer).join('、');
  ...
  elements.metaRow.innerHTML = [
    ...
    `<span class="pill">答案 ${{correctText}}</span>`,
  ].join('');
...
function importProgress(file) {{
  ...
  reader.onload = () => {{
    try {{
      ...
      state.progress = payload.progress && typeof payload.progress === 'object' ? payload.progress : payload;
      saveProgress();
      chooseNextQuestion();
    }} catch {{
